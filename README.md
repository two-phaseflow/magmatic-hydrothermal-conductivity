...
- **Hydrothermal domain**: generalized Archie's law for pore fluid combined with a surface-conduction term. Five surface-conduction parameterizations are available (Waxman & Smits, 1968; Revil et al., 2002; Lévy et al., 2018; and two hybrid dispatches).
...
| `verify_revil2002_fig7b.py` | Standalone validation: reproduces Revil (2002) Fig. 7b and compares bulk σ against the MT clay-cap range for WS / Revil / Lévy. |
...
| `'revil'` | Revil (2002) Eq. 7 + Eqs. 11–13 | Intrinsic clay surface conductivity `σ_S = (2/3)·ρ_g·β_s·CEC`. The bulk ρ uses the full Dukhin-number formulation with rollover across the isoconductivity point ξ₊ = 1 − t₊ ≈ 0.61 for NaCl, via `revil2002_bulk_conductivity()`. |
...
| `'waxman_smits_revil'` | WS (non-clay) + Revil 2002 (clay caps) | Dispatches Revil 2002 in regions listed in `config['clay_cap_regions']`. |
...
See `run_conductivity_test.py` for a complete example with plotting, and `verify_revil2002_fig7b.py` for validation of the Revil 2002 implementation against the paper's Fig. 7b.
...
- Revil, A. et al. (1998, 2002). *JGR* 103, 23925; *JGR* 107(B8), 2168.
